...
按“最终资产用途”优先匹配 Skill。以下名称由 Codex 的 Skill 发现机制解析，不要把它们当作相对于本文件的文件路径：

- `longs-imagegen-wechat-header` — 微信公众号文章头图
- `longs-imagegen-xiaohongshu-cover` — 小红书封面图
- `longs-imagegen-visual-explainer` — 概念图、关系图、流程图、生态图、解释型视觉化图
- `longs-imagegen-kids-educational-booklet` — 儿童教育小册子、学习册、生活能力册、方法训练册、连续多页图文教学资产
